 ...
 try{
  ...
  await context.route(origin+'/api/**',route=>{
   ...
   if(name==='/chats/'+chat.id){const continued=resumed?{continued_by:'next'}:{};return send({chat,messages:[
    ...
    ...(resumed?[{seq:4,sender:'user',kind:'continuation',text:'Continuing task',run_id:'next',created:now},{seq:5,sender:'piper',kind:'assistant',text:'I will pick up where the previous attempt stopped.',run_id:'next',created:now}]:[])
   ]});}
  ...
  await p.locator('.task-continuation').waitFor();
  assert.deepEqual(requests,[{}]);
  assert.equal(await p.locator('.task-continuation').innerText(),'Continuing task');
  ...
  for(const width of [1200,390]){
   ...
   for(const opening of [true,false]){
    const frames=await p.evaluate(async()=>{
     const history=document.querySelector('.task-recovery-history'),summary=history.querySelector('summary'),next=document.querySelector('.task-continuation');
  ...
  await p.reload();await p.locator('.task-continuation').waitFor();assert.equal(await p.locator('.task-recovery-history').evaluate(n=>n.open),false);
